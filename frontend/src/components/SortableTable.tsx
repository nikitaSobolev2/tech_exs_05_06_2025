...
const SortableTable: React.FC = () => {
  ...
  const fetchItems = useCallback(
    async (
      page: number,
      search: string,
      options?: { preserveOrderAndSelection?: boolean; signal?: AbortSignal }
    ) => {
  ...
  useEffect(() => {
    ...
    const loadInitialState = async () => {
      try {
        const stateResponse = await axios.get(`${API_URL}/items/state`, {
          signal: controller.signal,
        });
        const {
          selectedItemIds: serverSelectedIds /*, itemOrder: serverItemOrder*/,
        } = stateResponse.data;

        setSelectedIds(new Set(serverSelectedIds || []));

        await fetchItems(1, "", {
          preserveOrderAndSelection: true,
          signal: controller.signal,
        });
      } catch (error: unknown) {
        ...
        console.error(
          "An unexpected error occurred during initial state load:",
          error
        );
      }
    };
  ...
  const handleSelectionChange = (itemId: number, isSelected: boolean) => {
    ...
    axios
      .post(`${API_URL}/items/selection`, {
        selectedIds: Array.from(newSelectedIds),
      })
      .catch((err) =>
        console.error("Error updating selection on server:", err)
      );
  };
